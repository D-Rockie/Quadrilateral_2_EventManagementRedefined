    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "error" in crowd_data:
                ...
                default_lat, default_lon = 12.8225, 80.2250  # Default to SSN College, Kalavakkam, India
            ...
            fig = go.Figure(data=[
                go.Bar(
                    x=stall_names,
                    y=crowd_counts,
                    marker_color=['#006400' if count <= 1 else '#b3e6b3' if count <= 3 else '#ff9f40' if count <= 5 else '#ff5733' if count <= 7 else '#800000' for count in crowd_counts],  # Updated colors
                    width=0.4  # Slightly wider bars for better visibility
                )
            ])
            fig.update_layout(
                ...
                plot_bgcolor='#1a1a1a',  # Dark background to match your UI
                paper_bgcolor='#1a1a1a',  # Dark background for the chart area
                bargap=0.3,
                bargroupgap=0.1,
                font=dict(size=12, color='white'),  # White text for visibility on dark background
                height=400,
                width=600,
                showlegend=False
            )
            ...
            # Map integration using folium with debugging, centering on stall locations
            default_lat, default_lon = 12.8225, 80.2250  # Default to SSN College, Kalavakkam, India
            logger.info(f"Creating map with default location: {default_lat}, {default_lon}")

            # Calculate the average latitude and longitude of stalls for centering
            stall_lats = []
            stall_lons = []
            for stall, details in crowd_data.items():
                if stall != "default":  # Skip default stall for averaging
            ...
            if stall_lats and stall_lons:  # If there are real stalls, center on their average
            ...
            # Add markers for each stall, including the default if no real stalls exist
            has_markers = False
            for stall, details in crowd_data.items():
                try:
...
def delete_event(event_id, user_id):
    ...
    print(f"Checking deletion for event_id {event_id}: user_id = {user_id}, created_by = {event['created_by'] if event else None}")  # Debug log
